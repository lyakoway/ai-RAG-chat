...
| `demo_documents_load` | Загрузка демо-документов | Кнопка «Загрузить демо-документы» на пустой панели |
| `demo_documents_load_error` | Ошибка демо-пака | Сбой запроса /api/documents/demo |
...
| `mode_change` | Режим RAG / Агент / Поиск | Dropdown «Режим» |
| `vector_search` | Векторный поиск выполнен | Поиск в режиме «Векторный поиск» |
| `vector_search_error` | Ошибка векторного поиска | Сбой запроса /api/search |
...
Новый элемент UI — dropdown **«Режим»** (RAG Chat / AI Агент / Векторный поиск) в top bar и sidebar.

| Событие | Когда | Параметры |
|---------|--------|-----------|
| `mode_change` | Выбрали другой режим в dropdown | `from` (`rag` \| `agent` \| `search`), `mode` (новое значение) |
...
Третий режим — **«Векторный поиск»**: прямой семантический поиск по фрагментам
(fastembed + Chroma), без LLM.

| Событие | Когда | Параметры |
|---------|--------|-----------|
| `vector_search` | Успешный поиск (Enter / кнопка / suggestion) | `results_count`, `took_ms`, `category` |
| `vector_search_error` | Ошибка запроса /api/search | `message` (до 120 символов) |

Клик по результату дополнительно шлёт `source_click` → `viewer_open` (см. ниже).
...
| `demo_documents_load` | Кнопка «Загрузить демо-документы» | — |
| `demo_documents_load_error` | Ошибка загрузки демо-пака | `message` (до 120 символов) |
| `document_download` | Скачивание файла из панели документов | `filename` |
| `answer_feedback` | Оценка ответа 👍/👎 (повторный клик снимает — `value: clear`) | `value` (`up` \| `down` \| `clear`) |
| `followup_click` | Клик по follow-up подсказке под ответом | — |
| `voice_input` | Старт голосового ввода (кнопка микрофона) | `context` (`chat` \| `search`) |
| `provider_error_modal` | Модалка «Модель недоступна» показана (баланс/ключ провайдера) | `model`, `message` (до 120 символов) |
| `provider_error_contact` | Клик «Связаться с нами» в модалке провайдера | `model` |

При ошибке уходит только `*_error`, без парного success-события. Показ
`provider_error_modal` дополняет парный `chat_error` (тот же сбой стрима):
`chat_error` фиксирует сам сбой, `provider_error_modal` — что сбой распознан
как провайдерский и посетителю показано объяснение с контактом.
