...
| `mode_change` | Режим RAG / Агент | Переключатель режима |
...
**Приоритет (если не все сразу):** `chat_message_send` → `document_upload` → `source_click` → `viewer_open` → `chat_new`.
...
| `mode_change` | Переключатель RAG Chat / AI Агент | `mode` (`rag` \| `agent`) |
