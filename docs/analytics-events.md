...
| `agent_run_error` | Ошибка агента | Сбой / обрыв прогона в режиме AI Агент |
...
| `agent_run_error` | Ошибка стрима / ответа агента (`onError`) | `steps`, `model`, `message` (до 120 символов) |

В Метрике заведи JS-цели `mode_change`, `agent_run_done`, `agent_run_error` (условие «совпадает»).  
При ошибке уходит только `agent_run_error`, без `agent_run_done`. Отмена пользователем (Stop) событием не считается.
