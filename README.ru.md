...
ручного перебора PDF, Word и Excel.

Этот репозиторий — самостоятельное **личное демо** на публичном тестовом паке
(6 файлов → 12 чанков). Тот же класс задачи, что у production RAG-ассистента
по документам — **не** исходный код и не документы той системы. Цифры поиска
ниже совпадают со страницей кейса
[lyakoway.vercel.app/portfolio/rag-chat](https://lyakoway.vercel.app/portfolio/rag-chat)
(held-out ~180 запросов). Продовые корпуса — под NDA.

Пайплайн строился под три ограничения, и каждое из них ниже подтверждено замером:
...
1. **Гибрид BM25 + RRF оставляем** — dense-only Recall@1 53,2% на RU/EN-двойниках
   → **87,2%** у гибрида на held-out (~180 запросов).
2. **Реранкер отклоняем** — гибрид + cross-encoder Recall@1 **41,7%**
   (−45 п.п.) и **~2,9 с** сверху. Проверили и отказались.
3. **По умолчанию GLM-5.3-flash**, если есть ключ Z.ai — GLM-4.5-flash на том же
   пайплайне даёт TTFT 25–50 с. Сам пайплайн добавляет ~20 мс (<1%).
...
набор (~180 запросов)** со страницы кейса, не продовые документы. Индекс
собирается с нуля при каждом запуске.
...
Что я знаю о границах проекта — чтобы не откладывать ответы:

- **Корпус.** Цифры evaluation на этой странице — публичный демо-пак
  (6 файлов, 12 чанков) и held-out набор (~180 запросов) со страницы кейса:
  верхняя граница, не качество прода МТС. Продовые данные — под NDA.
