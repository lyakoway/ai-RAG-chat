...
47 золотых сценариев (RU + EN) по двуязычным демо-документам в четырёх
категориях — `fact` (прямой факт), `numeric` (цифры, лимиты, сроки),
`paraphrase` (перефраз без дословных ключей) и `cross-lingual` (смешанный язык
/ язык вопроса ≠ языка документа). Метрики Recall@k и MRR — на том же
...
Результаты на двуязычном корпусе (Recall@1 / Recall@3 / MRR@5 / задержка на
запрос, CPU, прогон 07.09.2026):
...
| Векторный + реранкер                 | TBD       | TBD      | TBD       | TBD         |
| Гибрид + реранкер                    | TBD       | TBD      | TBD       | TBD         |
...
английский документ (Recall@1 падает до 53.2%). BM25 различает лексику и вытягивает
правильный документ — гибрид даёт 87.2%. Реранкер же здесь **вредит**: cross-encoder
...
Расширенный набор **вскрыл и слабое место**: смешанные языковые запросы
(cross-lingual) дают 62.5% Recall@1 против 100% у прямых фактов — BM25 не
спасает, когда ключевые слова в другом языке. Следующий шаг: словарь синонимов
RU/EN и мультиязычный реранкер.
...
Результат на полном golden set — 47 сценариев (ответы и судья:
glm-4.5-flash, поиск — гибрид):
...
Ответов с оценкой ≤3 хотя бы по одной оси: 0 из 47 (на демо-корпусе).
