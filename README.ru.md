...
> Чат по документам **с цитатами-источниками** по вашим файлам (PDF, Word,
> Excel). Три режима рядом — **RAG Chat**, **AI-агент** и **Векторный поиск** —
> разница видна на одном и том же вопросе. Ответы опираются **только на
> загруженные документы** и содержат цитаты **с точной страницей**.
...
| Стоимость вопроса · GLM-5.3-flash            | ≈ $0,0005 (~20 тыс. на $1)  |
...
Пайплайн строился под три ограничения, каждое подтверждено замером ниже:
...
| Held-out поиск      | Recall@1 87,2% · Recall@3 97,9%                    | Вспомогательное число на публичном паке. Смешанный язык ещё открыт                              |
...
Подсвеченные строки — решения оставить / отклонить; абсолютные скоры под ними —
вспомогательные замеры, не claim.
...
4. **Observable AI systems** — tool-вызовы, шаги, ошибки, фидбек и латентность
   должны быть видимыми: SSE done/error, лента шагов агента, 👍/👎 в БД,
   латентность в логах.
...
набор (~180 запросов)** на публичном демо-паке. Индекс собирается с нуля при
каждом запуске.
